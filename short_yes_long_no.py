...
# ======================================================
BET_SIZE     = 5.0        # dollars per market
CAP_NO       = 0.40       # effective NO entry cap (equiv to selling YES at >= 0.60)
FEE_BPS      = 0          # 600 = 6.00%
SLIP_BPS     = 0          # 200 = 2.00%
START_BANK   = 5000.0
...
# ======================================================
#  SimMarket: MAKER LONG NO via selling YES
# ======================================================

class SimMarket:
    """
    We approximate you as a MAKER going LONG NO at a fixed cap price CAP_NO.

    normalize_trades() builds blocks from trades where:
        outcome == "yes" and side == "buy"
    i.e. taker is BUYING YES and hitting resting YES asks (including yours).

    For any such trade with price_yes >= CAP_YES = 1 - CAP_NO, we assume:
      - Your resting YES ask at CAP_YES was in the book and got filled.
      - Economically, that's identical to BUYING NO at price CAP_NO.
    """
    ...
    def take_first_no(self, t_from, dollars=100.0, max_no_price=None):
        """
        MAKER-at-cap LONG NO:

        - Look at all maker-long-NO blocks with time >= t_from
        - Blocks are derived from YES-buy trades with price_yes >= CAP_YES
        - We treat every fill as if it happened at NO price = max_no_price (CAP_NO)
        - You can buy up to dollars / max_no_price shares.
        """
        ...
        for b in self.blocks:
            if int(b.get("time", 0)) < t_from_ts:
                continue
            # maker-long-NO blocks are tagged as side="no_maker"
            if b.get("side") != "no_maker":
                continue
        ...
        if total_shares <= 0.0:
            print("    [SIM] No maker-long-NO liquidity under cap; no fill.")
            return 0.0, 0.0, 0.0, []
        ...
        print(
            f"    [SIM] maker LONG NO fill: "
            f"shares={target_shares:.2f}, spent_after={spent_after:.2f}, avg={avg_no:.4f}"
        )
...
def fetch_trades(market_dict):
    """
    Fetch ALL trades for a market using pagination, ensuring uniqueness.
    """
...
# ======================================================
#  Time + trade normalization: build maker-long-NO blocks from YES buys
# ======================================================
...
def normalize_trades(trades):
    """
    Build blocks representing *maker-long-NO* exposure created by your
    resting YES asks at an implied NO price CAP_NO.

    We interpret:
      outcome == "yes" and side == "buy"
    as:
      - A taker is BUYING YES.
      - They are hitting resting YES asks in the order book.
      - If their trade price >= CAP_YES (1 - CAP_NO), your 0.6 ask would
        have been fillable; we approximate that it gets filled at CAP_YES.

    So for each YES-buy trade with price_yes >= CAP_YES, we create a
    synthetic NO-long block at price_no = CAP_NO.
    """
    if not trades:
    ...
    kept_yes_buys = 0
    kept_yes_buys_ge_cap = 0
    min_yes_price = None
    max_yes_price = None

    cap_no = float(CAP_NO)
    cap_yes = 1.0 - cap_no  # e.g. CAP_NO=0.40 → CAP_YES=0.60

    for tr in trades:
        try:
            price   = float(tr["price"])
            size    = float(tr["size"])
            side    = str(tr.get("side", "")).strip().lower()
            outcome = str(tr.get("outcome", "")).strip().lower()
        except Exception:
            continue
        ...
        # We only care about taker BUYING YES → they hit resting YES asks.
        if not (outcome == "yes" and side == "buy"):
            continue

        kept_yes_buys += 1

        # Track YES trade price stats for debugging
        if min_yes_price is None or price < min_yes_price:
            min_yes_price = price
        if max_yes_price is None or price > max_yes_price:
            max_yes_price = price

        # Effective NO price if you hedge this with a short-YES position:
        # selling YES at price_yes is equivalent to buying NO at 1 - price_yes.
        eff_no_price = 1.0 - price

        # We want eff_no_price <= CAP_NO → price_yes >= CAP_YES
        if eff_no_price > cap_no:
            continue

        kept_yes_buys_ge_cap += 1
        ...
        # For the sim we treat fills as if they happened exactly at
        # NO price = cap_no and YES price = cap_yes.
        blocks.append({
            "time": ts,
            "side": "no_maker",     # maker-long-NO block
            "price_yes": round(cap_yes, 6),
            "price_no":  round(cap_no, 6),
            "shares": size,
            "notional_yes": round(size * cap_yes, 6),
            "notional_no":  round(size * cap_no, 6),
            "compressed": 1,
        })

    print(
        f"    [TRADES] raw={raw_count} yes-buy-trades={kept_yes_buys} "
        f"yes-buy>=cap_yes={kept_yes_buys_ge_cap} "
        f"min_yes_price={min_yes_price} max_yes_price={max_yes_price} "
        f"(CAP_YES={cap_yes:.3f}, CAP_NO={cap_no:.3f})"
    )

    blocks = sorted(blocks, key=lambda b: b["time"])
    print(f"    [TRADES] normalized maker-long-NO blocks={len(blocks)}")
    return blocks
...
def simulate_market_maker_no(cid, question, trades_raw, bank, all_pl, cap_no, fee_bps, slip_bps):
    """
    Shared core: simulate you as a MAKER going LONG NO at cap_no.
    ...
    if not trades:
        print("    [SIM] No valid maker-long-NO blocks; skipping.")
    ...
    print(f"    [SIM] starting maker LONG NO: bet={bet:.2f}, cap={cap_no:.3f}")
