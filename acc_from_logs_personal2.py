...
def main():
    ...
    while True:
        ...
        for i, (cid, meta) in enumerate(markets.items(), 1):
            q = meta["question"]
            baseline = _parse_iso_to_epoch(meta["time_found"]) or 0  # still stored, but not used as cutoff

            # Skip already-final closed markets (we assume spread won't change and already logged)
            ...
            # Look up any previous row/spread we have
            ...
            # Fetch meta & status (live Polymarket state)
            try:
            ...
            # ------------- Trade fetching strategy -------------
            # If the market is resolved now and we already have a spread from before,
            # DON'T re-fetch trades. Just reuse that cap_spread.
            if status in ("YES", "NO") and prev_spread is not None:
                print(f"    → market resolved now ({status}); reusing previous cap_spread, skipping trades fetch.")
                cap_spread = prev_spread
                ...
                # Rebuild cap spread from scratch each time (ignore any prev_spread)
        ...
        print(f"[WRITE] {len(closed_rows)} closed rows → {closed_path}")
