...
def fetch_all_trades_since(
    ...
    """
    Incremental trade fetch using OFFSET-based paging, with a time baseline:

      - We page with offset: 0, limit, 2*limit, ...
      - We only *keep* trades with ts >= since_epoch_s.
      - We de-dup by trade id.
      - We stop when:
          * a page is empty, OR
          * we've already reached >= since_epoch_s at least once, AND
            this page added 0 new trades >= since_epoch_s, OR
    ...
    seen_since_region = False  # becomes True once we see any trade with ts >= since_epoch_s
    ...
    for page_no in range(1, hard_cap_pages + 1):
        ...
        for t in page:
            ...
            ts = trade_ts(t)
            if ts < since_epoch_s:
                # Too old for this incremental pass; skip but keep paging forward.
                continue

            # We've now definitely entered the "since_epoch" region.
        ...
        # Once we're in the >= baseline region and add no new trades, we've exhausted
        # all trades since the baseline (API likely repeating pages or just done).
        if seen_since_region and page_added == 0:
            print("        [STOP] no new trades beyond baseline; stopping")
            break

        # Short page usually means end-of-history.
...
def main():
    ...
    while True:
        ...
        for i, (cid, meta) in enumerate(markets.items(), 1):
            ...
            # Fetch only new trades >= since_epoch using offset-based paging
            try:
