...
def fetch_trades(market_dict):
    """Simple full trade history (100 limit, no pagination)."""
...
def simulate_market_maker_no(cid, question, trades_raw, bank, all_pl, cap_no, fee_bps, slip_bps):
    ...
    If trades or status invalid, returns zeros and status "SKIP".
    """
    trades = normalize_trades(trades_raw)
    if not trades:
        return 0.0, 0.0, 0.0, "SKIP", False, 0.0
        ...
        try:
            for _ in range(BATCH_LIMIT):
                ...
                pnl, spent_after, shares, status, won, avg_ = simulate_market_maker_no(
                    cid, q, trades_raw, bank + pnl_batch, all_pl, CAP_NO, FEE_BPS, SLIP_BPS
                )

                # no status / no fill
                if status in ("SKIP", "NOFILL"):
                    idx += 1
                    continue
            ...
            # First Ctrl+C → soft interrupt (snapshot only)
            if not SOFT_INTERRUPTED_ONCE:
                ...
                write_status_report()
                continue
            # Second Ctrl+C → re-raise for hard exit
            raise
...
def run_historical(days_back: int):
    global SOFT_INTERRUPTED_ONCE
    ...
    print(f"Historical mode: days_back={days_back}, start_epoch={start_epoch}")
    ...
    while idx < total_markets:
        ...
        print("-" * 61)
        print(
            f"[HIST] batch #{batch_idx} | visited_this_batch: {visited_this_batch} "
            f"| bets_this_batch: {bets_this_batch} "
            f"| total_markets_done: {all_done} | batch P/L: {pnl_batch:.2f} "
            f"| total P/L: {all_pl:.2f} | bank: {bank:.2f}"
        )
        ...
        write_to_file(
            RUN_LOG_PATH,
            f"[HIST] batch:{batch_idx} | visited_total:{all_done} | bets_total:{bets_this_batch} "
            f"| total spent {spent:.2f} | batch P/L: {pnl_batch:.2f} | total P/L: {all_pl:.2f} "
            f"| bank: {bank:.2f} | last_createdAt: {last_created}"
        )
...
def main():
    ...
    parser.add_argument("--daysback", type=int, default=30, help="Days back for historical mode")
    ...
    parser.add_argument("--rps",      type=float, default=RPS_TARGET, help="Global HTTP requests per second target")
...
if __name__ == "__main__":
    try:
        ...
        # HARD INTERRUPT: second Ctrl+C bubbles up here
