...
def fetch_trades(market_dict):
    """Simple full trade history (100 limit, no pagination) with verbose prints."""
    cid = market_dict["conditionId"]
    print(f"    [TRADES] fetching trades cid={cid[:10]}…")
    try:
        resp = safe_get(
            DATA_TRADES,
        ...
        payload = resp.json()
        n = len(payload or [])
        print(f"    [TRADES] got {n} raw trades for cid={cid[:10]}")
        return payload
    except Exception as e:
        print(f"    [TRADES WARN] {cid[:10]}… {e}")
        return None
