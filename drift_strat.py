...
# ======================================================
#  SimMarket: maker-at-cap NO (LONG NO)
# ======================================================

class SimMarket:
    """
    We approximate you as a MAKER going LONG NO.

    normalize_trades() only constructs blocks from trades where:
        outcome == "no" and side == "sell"
    i.e. the taker sells NO into bids → makers on that book are BUYING NO.
    Those makers are long NO; that's the position we want to simulate.
    """
    ...
    def take_first_no(self, t_from, dollars=100.0, max_no_price=None):
        ...
        - Look at all NO-maker blocks with price_no <= cap and time >= t_from
        - Those blocks represent SELL NO takers hitting bids → makers BUY NO.
        - You place a limit NO-BUY at cap; you can buy up to dollars/cap shares.
        - Cost is shares * cap (plus fee+slip). Payout is 1 if NO wins.
        """
        ...
        for b in self.blocks:
            if int(b.get("time", 0)) < t_from_ts:
                continue
            # side=="no_maker" (see normalize_trades) means "maker-long-NO block"
        ...
        if total_shares <= 0.0:
            print("    [SIM] No NO-maker liquidity under cap; no fill.")
            return 0.0, 0.0, 0.0, []
        ...
        print(f"    [SIM] maker LONG NO fill: shares={target_shares:.2f}, spent_after={spent_after:.2f}, avg={avg_no:.4f}")
...
# ======================================================
#  Time + trade normalization (SIMPLE, maker-long-NO)
# ======================================================
...
def valid_trade(trade, min_spend=2, extreme_price=0.05, min_extreme_notional=20.0):
    """
    We only care about outcome='no' & side='sell' trades for maker-long-NO.
    cost is interpreted as price * size (USDC spent on NO).
    """
    if not trade:
        return False
...
def normalize_trades(trades):
    """
    Build blocks representing *maker-long-NO* liquidity.

    We interpret:
      outcome == "no" and side == "sell"
    as:
      taker is selling NO into resting NO bids
      → the maker(s) on the other side are BUYING NO (long NO).

    Each such trade becomes a single NO block:
      - price_no  = trade.price
      - price_yes = 1 - trade.price
      - shares    = trade.size
    """
    if not trades:
    ...
    for tr in trades:
        ...
        # Only taker SELLS NO -> maker BUYS NO (long NO)
        if not (outcome == "no" and side == "sell"):
            continue
        ...
        p_no = price
        if p_no > CAP_NO:
            # would be NO-side but above your cap
            if min_no_price is None or p_no < min_no_price:
                min_no_price = p_no
            if max_no_price is None or p_no > max_no_price:
                max_no_price = p_no
            continue
        ...
        blocks.append({
            ...
            "shares": size,
            "notional_yes": 0.0,
            "notional_no": round(size * p_no, 6),
            "compressed": 1,
        })

    print(
        f"    [TRADES] raw={raw_count} no-sell-trades={kept_no_sells} "
        f"no-sell-<=cap={kept_under_cap} "
        f"min_no_price={min_no_price} max_no_price={max_no_price}"
    )
