...
def make_session():
    ...
    s.headers.update({"User-Agent": "pm-maker-backtest/2.0-debug"})
    return s
...
def _rps_on_429():
    global _RPS_SCALE, TOTAL_429
    TOTAL_429 += 1
    _RPS_SCALE = max(_RPS_MIN, _RPS_SCALE * 0.7)
    print(f"[RPS] 429 hit → RPS_SCALE={_RPS_SCALE:.3f} TOTAL_429={TOTAL_429}")
...
def _rps_recover(dt_sec: float):
    ...
    if _RPS_SCALE != old:
        print(f"[RPS] recovering → RPS_SCALE={_RPS_SCALE:.3f}")
...
def http_get_with_backoff(url, *, params=None, timeout=20, max_tries=8):
    ...
    while True:
        ...
        try:
            ...
            print(f"[HTTP] Exception {e} → sleep {back:.1f}s")
        ...
        if r.status_code == 429:
            ...
            print(f"[HTTP] 429 → sleeping {sleep_s:.1f}s (Retry-After={ra})")
            time.sleep(sleep_s)
        ...
        if 500 <= r.status_code < 600:
            print(f"[HTTP] {r.status_code} server error → sleep {back:.1f}s")
            time.sleep(back + random.random()*0.2)
            back = min(back*1.7, 20.0)
            tries += 1
            if tries >= max_tries:
        ...
        print(f"[HTTP] hard error {r.status_code} → raising")
        r.raise_for_status()


# ======================= Heartbeat ================================
def heartbeat(current_idx: int, total: int, last_cid: str):
    """
    Global heartbeat: prints every HEARTBEAT_EVERY seconds to show that
    the script is alive and where it is.
    """
    ...
    if now - _LAST_HEARTBEAT < HEARTBEAT_EVERY:
        return
    _LAST_HEARTBEAT = now
    cid_short = (last_cid or "")[:10] + "…" if last_cid else "(none)"
    print(
        f"[HEARTBEAT] {dt_iso()} | market {current_idx}/{total} "
        f"last_cid={cid_short} RPS_SCALE={_RPS_SCALE:.3f} "
        f"bucket={_bucket:.2f} requests={REQUEST_COUNT} 429s={TOTAL_429}"
    )
...
def fetch_market_full_by_cid(cid: str) -> dict:
    ...
    try:
        ...
        print(f"[WARN fetch_market_full_by_cid] {cid}: {e}")
        MARKET_META_CACHE[cid] = {}
        return {}
...
def fetch_trades_page_ms(cid: str, limit=TRADES_PAGE_LIMIT, starting_before_s=None, offset=None):
    ...
    return http_get_with_backoff(DATA_TRADES, params=params, timeout=20).json() or []
...
def build_maker_buckets(cid: str, since_epoch_s: int, caps: List[float], max_notional: float):
    ...
    DEBUG: prints per-page info so you can see if it's stuck.
    """
    ...
    print(f"    [BUCKET] start cid={cid[:10]}… since={since_epoch_s}")

    for page_idx in range(1, 10001):
        data = fetch_trades_page_ms(cid, limit=TRADES_PAGE_LIMIT, starting_before_s=starting_before)
        pages += 1
        n = len(data)
        total_trades += n

        print(f"      [PAGE {page_idx:4d}] trades={n} starting_before={starting_before} elapsed={time.time()-t0:5.1f}s")
        ...
        if all(st["under_cap_seen"] >= max_notional - 1e-9 for st in caps_state.values()):
            print("        → all caps reached max_notional, stopping.")
            break

        oldest_ts = min(_to_epoch_any(t.get("timestamp") or t.get("time") or t.get("ts") or 0) or 0 for t in data)
        print(f"        → oldest_ts={oldest_ts} since={since_epoch_s}")
        if oldest_ts <= since_epoch_s:
            print("        → reached baseline since_epoch, stopping.")
            break

        starting_before = oldest_ts
        if last_len == len(data) and not progressed:
            print("        → stagnation (same len, no progress), stopping.")
            break
        last_len = len(data)

    print(f"    [BUCKET DONE] cid={cid[:10]}… pages={pages} trades={total_trades} elapsed={time.time()-t0:5.1f}s")
...
# ======================= Main ====================================
def main():
    ap = argparse.ArgumentParser(description="Polymarket NO-maker backtest (multi-cap buckets + single view + sweep) [debug].")
    ...
    while True:
        ...
        total_markets = len(uniq)

        for i, (cid, meta) in enumerate(uniq.items(), 1):
            heartbeat(i, total_markets, cid)
            ...
            print(f"\n[{i}/{len(uniq)}] {q}  (cid={cid[:10]}…)  since={meta['time_found']}")
